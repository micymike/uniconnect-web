...
const Hero = () => {
  ...
  const parallaxScale = Math.max(0, 1 - scrollPosition / 500);
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-64 h-64 bg-primary/20 rounded-full filter blur-3xl animate-pulse" style={{ transform: `scale(${parallaxScale})` }}></div>
        <div className="absolute bottom-1/3 right-1/3 w-96 h-96 bg-blue-500/10 rounded-full filter blur-3xl animate-pulse" style={{ animationDelay: '1s', transform: `scale(${parallaxScale})` }}></div>
      </div>
